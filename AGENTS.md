...
> This file is the entry point for any AI agent (Codex, GitHub Copilot, etc.) working on this repo. Read it cold; you'll have full context after.
...
| Component | What | Why |
|--|--|--|
| Node 22, ESM, top-level await | Runtime | User's stack rule (AGENTS.md global). `tsx` for `.ts` execution. |
...
- **TypeScript / Next.js / Vercel for frontend; Railway for backend.** From their global AGENTS.md.
- **Backend uses Supabase, not Railway DB.** From global AGENTS.md (no DB used yet, but if we add one).
...
- **Read the user's global AGENTS.md** at `~/.Codex/AGENTS.md` for style rules that override defaults.
